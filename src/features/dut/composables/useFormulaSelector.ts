...
export const FORMULA_DEFINITIONS: Record<FormulaType, FormulaMetadata> = {
  ...
  EVM: {
    ...
    description:
      'Error Vector Magnitude scoring. Lower EVM values are better. Scores 6-10 for within spec, with bonus for excellent values.',
  ...
  FREQ: {
    ...
    description:
      'Linear scoring based on deviation from target frequency. Score decreases linearly from target to spec limits.',
    formula_latex:
      '\\text{Score} = 10 \\times \\left(1 - \\frac{|\\text{actual} - \\text{target}|}{\\text{span}}\\right)',
    usedFor: ['FREQ'],
    category: 'backend',
  },
  PER: {
  ...
  PA_ADJUSTED_POWER: {
    ...
    description:
      'PA power adjustment scoring. Target is 0 (ideal calibration). Uses 5dB threshold by default.',
    formula_latex:
      '\\text{Score} = 10 \\times \\left(1 - \\frac{|\\text{actual}|}{\\text{threshold}}\\right)',
    usedFor: ['PA'],
    category: 'backend',
  },
  PA_POW_DIF_ABS: {
    ...
    description:
      'PA power difference absolute (calibration delta). Target is 0. Linear scoring from 0 to threshold.',
    formula_latex:
      '\\text{Score} = 10 \\times \\left(1 - \\frac{|\\text{actual}|}{\\text{threshold}}\\right)',
    usedFor: ['PA'],
    category: 'backend',
  ...
  BOUNDED: {
    ...
    description:
      'Standard bounded measurement scoring. Score based on position within USL/LSL limits and deviation from target.',
  ...
  CUSTOM: {
    ...
    description:
      'User-defined LaTeX formula with custom parameters. Variables: x (actual), usl, lsl, target',
...
export function useFormulaSelector() {
  ...
  const activeFormulaStats = computed(() => {
    ...
    return {
      ...
      activeCategories,
      hasUniversal:
        universalFormula.value.enabled && universalFormula.value.formulaType !== 'SYSTEM',
  ...
  function extractCategory(testItem: string): string | null {
    ...
    if (
      testItemUpper.includes('PA') &&
      (testItemUpper.includes('ADJUSTED_POW') || testItemUpper.includes('ADJUSTED_POWER'))
    ) {
      return 'PA'
    }
   ...
   */
  // biome-ignore lint/suspicious/noExplicitAny: Backend results have dynamic object/array format
  function applyFormulaSelectionToResults(results: any[]): any[] {
    ...
    return results.map((result) => {
      const testItem =
        typeof result === 'object' && 'test_item' in result
          ? result.test_item
      ...
      // biome-ignore lint/suspicious/noExplicitAny: Measurement is polymorphic (object or destructured array)
      let measurement: any
...
function calculateBoundedScore(
  actual: number,
  usl: number | null,
  lsl: number | null,
  target: number,
