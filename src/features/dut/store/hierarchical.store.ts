...
export const useHierarchicalStore = defineStore('hierarchical', () => {
  ...
  async function fetchHierarchicalAnalysis(request: HierarchicalRequest): Promise<void> {
    ...
    try {
      ...
      if (response.errors.length > 0) {
        error.value = `${response.errors.length} DUT(s) failed analysis: ${
          response.errors.map(e => e.dut_isn).join(', ')
        }`
      }
      
      // Auto-select DUTs for comparison (max 5)
      selectedDUTs.value = response.results
        .slice(0, 5)
        .map(r => r.dut_isn)
  ...
  function selectAllDUTs(): void {
    selectedDUTs.value = results.value
      .slice(0, 5)
      .map(r => r.dut_isn)
  }
