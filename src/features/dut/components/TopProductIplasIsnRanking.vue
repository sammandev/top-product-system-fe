  ...
  <v-card elevation="2">
    <v-card-title class="d-flex align-center justify-space-between bg-secondary">
      ...
      <div class="d-flex align-center gap-2">
        ...
        <v-btn v-if="totalRecords > 0" color="primary" variant="tonal" size="small" prepend-icon="mdi-file-export"
    ...
    <v-tabs v-model="activeISNTab" color="primary" bg-color="grey-lighten-4" slider-color="primary"
      ...
      <v-tab v-for="(isnGroup, index) in isnGroups" :key="isnGroup.isn" :value="index">
        ...
        {{ isnGroup.isn }}
        <v-badge v-if="isnGroup.errorCount > 0" :content="isnGroup.errorCount" color="error" inline class="ml-1" />
      </v-tab>
    </v-tabs>
    ...
    <v-window v-model="activeISNTab">
      <v-window-item v-for="(isnGroup, isnIndex) in isnGroups" :key="isnGroup.isn" :value="isnIndex">
        ...
        <!-- Station Sub-tabs -->
        <v-tabs v-model="activeStationTabs[isnIndex]" color="secondary" bg-color="white" slider-color="secondary"
          :show-arrows="isnGroup.stations.length > 5" density="compact">
          <v-tab v-for="(station, stationIndex) in isnGroup.stations" :key="station.stationName" :value="stationIndex"
            class="text-caption">
            ...
            {{ station.displayName }}
            <v-badge v-if="station.errorCount > 0" :content="station.errorCount" color="error" inline class="ml-1" />
          </v-tab>
        </v-tabs>
          ...
          <v-window-item v-for="(station, stationIndex) in isnGroup.stations" :key="station.stationName"
            ...
            <v-card-text class="pa-3 bg-grey-lighten-5">
              <v-row dense align="center">
                <v-col cols="12" md="4">
                  <v-text-field v-model="searchFilters[`${isnIndex}-${stationIndex}`]" label="Search records"
                    prepend-inner-icon="mdi-magnify" variant="outlined" density="compact" hide-details clearable
                    placeholder="Device ID, error..." />
                </v-col>
                <v-col cols="12" md="3">
                  <v-select v-model="statusFilters[`${isnIndex}-${stationIndex}`]" :items="statusOptions" label="Status"
                    variant="outlined" density="compact" hide-details clearable />
                </v-col>
                <v-col cols="auto" class="ml-auto">
                  <v-btn-toggle v-model="selectAllToggles[`${isnIndex}-${stationIndex}`]" color="primary"
                    density="compact">
            ...
            <!-- Data Table -->
            <v-data-table :headers="headers" :items="getFilteredRecords(isnIndex, stationIndex, station.records)"
              :items-per-page="10" density="comfortable" hover class="ranking-table">
              <template #item="{ item, index }">
                <tr :key="`${isnIndex}-${stationIndex}-${index}`" :class="getRowClass(item, index)"
                  class="cursor-pointer" @click="handleRowClick(item)">
                  <td class="text-center">
                    <v-checkbox-btn v-model="selectedRecordsMap[getRecordKey(item)]" @click.stop density="compact" />
                  </td>
                  <td class="text-center">
                  ...
                  <td>
                    ...
                    <template v-else-if="getRecordScore(item) !== null">
                      <v-chip size="small" :color="getScoreColor(getRecordScore(item)!)" variant="flat"
                        class="font-weight-bold">
                        {{ (getRecordScore(item)! * 10).toFixed(2) }}
                      </v-chip>
                    </template>
                    <template v-else>
...
import type { IsnSearchData } from '@/features/dut-logs/api/iplasApi'
