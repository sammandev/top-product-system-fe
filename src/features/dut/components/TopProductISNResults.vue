...
import type { TopProductResult, TopProductError, TopProductMeasurement, TopProductStationResult, ScoreBreakdown } from '../types/dutTopProduct.types'
...
function parseMeasurements(data: Array<any>): TopProductMeasurement[] {
    ...
    for (let i = 0; i < data.length; i++) {
        ...
        if (isObjectFormat) {
            ...
            breakdown = item.score_breakdown && typeof item.score_breakdown === 'object'
...
const enhancedResults = computed(() => {
    return props.results.map(result => ({
        ...
        test_result: result.test_result.map(station => ({
            ...
            measurement_count: station.metadata?.measurement_count || station.data?.length || 0,
...
const measurementHeaders = computed(() => [
    ...
    { title: 'Δ Meas. & Target', key: 'delta_actual_target', sortable: false, align: 'center' as const },
...
const failedDUTsCount = computed(() => {
    return enhancedResults.value.filter(result => {
        ...
        return result.test_result.some(station =>
...
const filteredMeasurements = computed(() => {
    ...
    if (limitFilter.value) {
        filtered = filtered.filter(m => {
            ...
            const withinLimits = (m.lsl === null || actual >= m.lsl) &&
                (m.usl === null || actual <= m.usl)
...
const comparisonHeaders = computed(() => {
    ...
    return [...baseHeaders, targetHeader, ...measuredHeaders, actMaxDiffHeader, ...deltaHeaders, ...scoreHeaders]
})
...
function measureLockedColumns(tableEl: HTMLElement | null, lockedKeys: string[], setter: (map: Record<string, number>) => void) {
    if (!tableEl) return
...
const comparisonData = computed(() => {
    ...
    if (firstResult) {
        const firstStation = firstResult.test_result.find(s => s.station_name === selectedCompareStation.value)
        if (firstStation) {
    ...
    enhancedResults.value.forEach(result => {
        ...
        measurements.forEach(m => {
            ...
            if (m.actual !== null && m.actual !== '' && m.target !== null) {
                ...
                if (!isNaN(measuredNum) && !isNaN(targetNum)) {
                    testItemMap.get(m.test_item)![`delta_mt_${result.dut_isn}`] = measuredNum - targetNum
                }
            }
        })
    })
    ...
    const comparisonItems = Array.from(testItemMap.values()).map(item => {
        ...
        const measuredMaxDiff = measuredValues.length > 1
    ...
    if (comparisonLimitFilter.value) {
        filtered = filtered.filter(item => {
            ...
            return enhancedResults.value.some(result => {
                ...
                const measuredNum = parseFloat(measured)
                if (isNaN(measuredNum)) return false

                const withinLimits = (item.lsl === null || measuredNum >= item.lsl) &&
...
watch([measurementLockedColumns, filteredMeasurements], measureMeasurementWidths, { immediate: true })
watch([comparisonLockedColumns, filteredComparisonData], measureComparisonWidths, { immediate: true })

// Watch for locked columns changes specifically to ensure proper width measurement
watch(measurementLockedColumns, () => {
...
function getActualValueColor(measurement: TopProductMeasurement): string {
    ...
    if (isNaN(actual)) return 'default'

    const withinLimits = (measurement.lsl === null || actual >= measurement.lsl) &&
...
function getMeasuredValueColor(measuredValue: any, usl: number | null, lsl: number | null): string {
    ...
    const withinLimits = (lsl === null || actual >= lsl) &&
        (usl === null || actual <= usl)
