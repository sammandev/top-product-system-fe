<template>
    <v-dialog :model-value="modelValue" :fullscreen="isFullscreen" :max-width="isFullscreen ? undefined : 1400"
            ...
            <v-card-text class="pa-4 flex-grow-1 d-flex flex-column" style="overflow: hidden;">
                ...
                <template v-else>
                    ...
                    <v-row dense class="mb-3 flex-shrink-0">
                        <v-col cols="12" md="3">
                            <v-text-field v-model="searchQuery" label="Search (Regex)" placeholder="e.g. ^RF.*|Power"
                                prepend-inner-icon="mdi-regex" variant="outlined" density="compact" hide-details
                                clearable />
                        </v-col>
                        <v-col cols="6" md="2">
                        ...
                        <v-col cols="6" md="3" class="d-flex align-center gap-2">
                            <v-btn color="info" variant="tonal" prepend-icon="mdi-tune-variant"
                                ...
                                <v-badge
                                    ...
                                    color="warning" inline class="ml-1" />
                            </v-btn>
                        </v-col>
                        <v-col cols="6" md="2" class="d-flex align-center">
                            <v-btn color="success" variant="tonal" prepend-icon="mdi-microsoft-excel" block
                                @click="exportToExcel" :loading="exporting">
                                Export
                            </v-btn>
                        </v-col>
                    </v-row>
                    ...
                    <v-chip-group v-model="comparisonFilter" mandatory class="mb-4 flex-shrink-0">
                        ...
                        <v-chip value="mismatch" color="error" variant="flat">Mismatch Items ({{ mismatchCount
                            }})</v-chip>
                        <v-chip value="upload-only" color="warning" variant="flat">Uploaded Items ({{ uploadOnlyCount
                            }})</v-chip>
                        <v-chip value="iplas-only" color="info" variant="flat">iPLAS Items ({{ iplasOnlyCount
                            }})</v-chip>
                        <v-chip value="all" color="primary" variant="outlined">All ({{ comparisonItems.length
                            }})</v-chip>
                    </v-chip-group>
                    ...
                    <v-data-table :headers="comparisonHeaders" :items="filteredComparisonItems" :items-per-page="50"
                        ...
                        <template #item.upload_score="{ item }">
                            <v-chip v-if="item.upload_score !== null" :color="getScoreColor(item.upload_score)"
                                size="small" class="cursor-pointer" @click="showScoreBreakdown(item, 'upload')">
                                {{ item.upload_score?.toFixed(2) }}
                            </v-chip>
                            <span v-else class="text-medium-emphasis">-</span>
                        </template>
                        <template #item.iplas_score="{ item }">
                            <v-chip v-if="item.iplas_score !== null" :color="getScoreColor(item.iplas_score)"
                                size="small" class="cursor-pointer" @click="showScoreBreakdown(item, 'iplas')">
    ...
    <v-dialog v-model="showBreakdownDialog" :fullscreen="breakdownFullscreen"
        ...
        <v-card v-if="breakdownItem">
            <v-card-title class="d-flex align-center bg-info">
                ...
                <span class="text-white">Score Breakdown - {{ breakdownSource === 'upload' ? 'Uploaded' : 'iPLAS'
                    }}</span>
                <v-spacer />
                ...
                <v-table density="compact">
                    <tbody>
                        ...
                        <tr>
                            ...
                            <td>{{ breakdownSource === 'upload' ? breakdownItem.upload_value : breakdownItem.iplas_value
                                }}
                            </td>
                        </tr>
                        <tr>
                            ...
                            <td>
                                <v-chip
                                    ...
                                    size="small">
                                    {{ (breakdownSource === 'upload' ? breakdownItem.upload_score :
                                    breakdownItem.iplas_score)?.toFixed(2) }}
                                </v-chip>
                            </td>
                        </tr>
